package com.aac.base.widget.toast;
...
class StyleableToast implements OnToastFinished {
    ...
    private static final int toast_bottom_margin_dp = 40;
    ...
    private StyleableToast(Builder builder) {
        ...
        this.isSetView = builder.isSetView;
    ...
    public View getToastLayout() {
        ...
        if (drawable > 0) {
            ...
            toastLayout.setPadding(verticalPadding + 35, verticalPadding + 5, verticalPadding + 35, verticalPadding + 5);
        }
    ...
    private TextView getTextView() {
        ...
        textView.setTextSize(TypedValue.COMPLEX_UNIT_SP, mTextSize == 0 ? DEFAULT_TEXT_SIZE : mTextSize);
        ...
        if (drawable > 0) {
            ...
            //previous: 18x2  + 8
            int leftPadding = (int) Utils.getTypedValueInDP(context, 22 * 2 + 5);
            int rightPadding = (int) Utils.getTypedValueInDP(context, 5);
            ...
            textView.setPadding(0, leftPadding, 0, rightPadding);
        }
    ...
    private ImageView getIcon() {
        ...
        if (drawable > 0) {
            ...
            //previous 18:
            int marginLeft = (int) Utils.getTypedValueInDP(context, 5);
            int maxHeightVal = (int) Utils.getTypedValueInDP(context, 40);
            int maxWidthVal = (int) Utils.getTypedValueInDP(context, 40);
...
//            layoutParams.setMargins(0,marginLeft,  0, 0);
    ...
    public void showView(int gravity, int offx, int offy) {
    ...
    public void show() {
        if (toast == null) {
            ...
            toast.setView(getToastLayout());
            toast.setDuration(duration);
            toast.setGravity(Gravity.BOTTOM, 0, ConvertUtils.dp2px(toast_bottom_margin_dp));
            toast.show();
            oneTime = System.currentTimeMillis();
        } else {
            twoTime = System.currentTimeMillis();
            if (toastMsg.equals(oldMsg)) {
                if (twoTime - oneTime > Toast.LENGTH_SHORT) {
                    toast.show();
                }
            } else {
                oldMsg = toastMsg;
                setToastMsg(toastMsg);//getToastLayout里面含有msg的更新信息
                toast.setView(getToastLayout());
                toast.setDuration(duration);
                toast.setGravity(Gravity.BOTTOM, 0, ConvertUtils.dp2px(toast_bottom_margin_dp));
                toast.show();
    ...
    public void show(int gravity, int offX, int offY) {
        if (toast == null) {
            toast = new Toast(context);
            toast.setView(getToastLayout());
            toast.setDuration(duration);
    ...
    public static final class Builder {
        ...
        public Builder(Context context, boolean isSetView) {
            this.context = context.getApplicationContext();
