...
public class CommonFileCallback implements Callback {
	 ...
	 */
	protected final String NETWORK_ERROR = "-1"; // the network relative error
	protected final String IO_ERROR = "-2"; // the JSON relative error
	...
	public void onFailure(final Call call, final IOException ioexception) {
		mDeliveryHandler.post(new Runnable() {
			@Override
			public void run() {
				mListener.onFailure(new OkHttpException(NETWORK_ERROR,
						"数据请求异常，请检查网络设置！"));
			}
		});
	}
		...
		mDeliveryHandler.post(new Runnable() {
			...
			public void run() {
				if (file != null) {
					...
					mListener
							.onFailure(new OkHttpException(IO_ERROR, "文件下载失败"));
				}
			}
		});
	}
	...
	private File handleResponse(Response response) {
		...
		try {
			...
			// file.mkdirs();
			// }		
			File file2 = new File("/sdcard/票据管理/");
			if (!file2.exists()) {
				file2.mkdirs();
			}
			file = new File("/sdcard/票据管理/" + mFilePath);
